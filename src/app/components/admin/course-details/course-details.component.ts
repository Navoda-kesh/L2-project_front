...
export class CourseDetailsComponent implements OnInit {
  ...
  constructor(private courseDetailsService: CourseDetailsService, private enrollmentservice:CourseService, 
    private adminuserservice:AdminUserControlService, private javaService: FilesJavaForBeginnersService,
    private payService: PayhereService) { }
  ...
  onDelete(_id: string, form: NgForm, course: string) {
    if (confirm('Are you sure to delete this record ?') == true) {
      this.courseDetailsService.deleteCourse(_id).subscribe(() => {
        ...
        this.deletecourseMaterials(course);
        this.deletepays(course);
        console.log('done');
  ...
  deletecourseMaterials(course:string){
    this.javaService.deleteFilesofCourse(course).then((result)=>{
  ...
  deletepays(course:string){
    this.payService.deletePaysForCourseDelete(course).then((result)=>{
